...
stateDiagram-v2
    ...
    state "Deep Sleep (Low Power)" as DeepSleep
    
    HardwareInit --> IdentifyWakeup: Init Drivers (NVS, WiFi, PMIC)
    ...
    state ManualMode {
        [*] --> SetFlag
        SetFlag --> SwitchCh: Switch Channel
        SwitchCh --> [*]
    }
    ...
    state WorkLoop {
        ...
        state "Scanning (Blocking Wait)" as WaitState
        note right of WaitState
            Wait 30s OR 
            Signal via TaskNotify
        end note

        StartScan --> WaitState: ble_scan_start()
        
        WaitState --> StartScan: Button Interrupt\n(Switch Channel & Reset Timer)
        WaitState --> LoopDecision: Timeout (30s)
        
        state LoopDecision <<choice>>
        LoopDecision --> StartScan: Still In Schedule\n(AND Not Manual)
        LoopDecision --> [*]: Manual Mode Done\nOR Schedule Ended
    }

    WorkLoop --> DeepSleep: Calculate Next Slot
    DeepSleep --> HardwareInit: Wakeup Triggered
```
